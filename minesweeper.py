    ...
    def add_knowledge(self, cell, count):
        ...
        for di in range(-1, 2):
            for dj in range(-1, 2):
                ...
                if 0 <= ni < self.height and 0 <= nj < self.width:
                    neighbor = (ni, nj)
                    # Only include undetermined cells (not known safe or mine)
                    if neighbor not in self.safes and neighbor not in self.mines:
                        neighbors.add(neighbor)
        
        # Create new sentence if there are undetermined neighbors
        if neighbors:
        ...
        while changed:
            ...
            for sentence in self.knowledge[:]:  # Use slice to avoid modification during iteration
                # Get known mines and safes from current sentence
            ...
            # Check for subset relationships and add new sentences
            new_sentences = []
            for i, sentence1 in enumerate(self.knowledge):
                for j, sentence2 in enumerate(self.knowledge):
                    if i != j and sentence1.cells.issubset(sentence2.cells):
                        # sentence1 is a subset of sentence2
